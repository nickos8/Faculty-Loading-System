...
<div class="max-w-3xl mx-auto px-4 py-6 space-y-6">
   ...
   {{-- Glass Banner Header (Create Section) --}}
<div class="rounded-3xl border border-white/40 bg-white/60 backdrop-blur-xl shadow-sm">
    <div class="p-6 sm:p-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
            <h1 class="text-3xl font-bold text-slate-900">Create Section</h1>
            <p class="text-sm text-slate-600 mt-1">Add a new class section</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white/70 backdrop-blur shadow-sm">
        <form method="POST" action="{{ route('sections.store') }}" class="p-6 space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="year_level" type="number"
                           value="{{ old('year_level', optional($default)->year_level) }}"
                           class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-200">
                    @error('year_level')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input name="term_no" type="number"
                           value="{{ old('term_no', optional($default)->term_no) }}"
                           class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-200">
                    @error('term_no')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="pt-2 flex items-center justify-end gap-2">
                ...
                <button type="submit"
                        class="rounded-xl bg-slate-900 px-5 py-2 text-white text-sm font-medium shadow-sm hover:bg-slate-800">
                    Create Section
                </button>
            </div>
        </form>
